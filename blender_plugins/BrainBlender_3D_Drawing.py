#    BrainBlender_3D_Drawing (C) 2018, Tom Boissonnet
#    Adapted from NeuroMorph_3D_Drawing.py (C) 2017,  Anne Jorstad
#
...
# Define properties
bpy.types.Scene.bb_ap_side = bpy.props.FloatProperty \
      (
        name = "AP",
        description = "Antero-Posterior-dimension of image stack (microns)",
        default = 13.2
      )
bpy.types.Scene.bb_ml_side = bpy.props.FloatProperty \
      (
        name = "ML",
        description = "Median-Lateral-dimension of image stack (microns)",
        default = 11.4
      )
bpy.types.Scene.bb_dv_side = bpy.props.FloatProperty \
      (
        name = "DV",
        description = "Dorso-Ventral-dimension of image stack (microns)",
        default = 8
      )

bpy.types.Scene.bb_image_ext_DV = bpy.props.StringProperty \
      (
        name = "extDV",
        description = "Image Extension DV",
        default = ".tif"
      )
bpy.types.Scene.bb_image_ext_AP = bpy.props.StringProperty \
        (
        name = "extAP",
        description = "Image Extension AP",
        default = ".tif"
    )
bpy.types.Scene.bb_image_ext_ML = bpy.props.StringProperty \
        (
        name="extML",
        description="Image Extension ML",
        default=".tif"
    )
bpy.types.Scene.bb_image_path_DV = bpy.props.StringProperty \
      (
        name = "Source-DV",
        description = "Select location of the DV stack images",
        default = "/"
      )
bpy.types.Scene.bb_image_path_AP = bpy.props.StringProperty \
        (
        name = "(Source-AP)",
        description = "Select location of the AP stack images",
        default = "/"
    )
bpy.types.Scene.bb_image_path_ML = bpy.props.StringProperty \
        (
        name="(Source-ML)",
        description="Select location of the Y stack images",
        default="/"
    )
bpy.types.Scene.bb_file_min_DV = bpy.props.IntProperty \
      (
        name = "file_min_DV",
        description = "min DV file number",
        default = 0
      )
bpy.types.Scene.bb_file_min_AP = bpy.props.IntProperty \
        (
        name = "file_min_AP",
        description = "min AP file number",
        default=0
    )
bpy.types.Scene.bb_file_min_ML = bpy.props.IntProperty \
        (
        name="file_min_ML",
        description="min AP file number",
        default=0
    )
bpy.types.Scene.bb_imagefilepaths_dv = bpy.props.CollectionProperty(type=bpy.types.PropertyGroup)
bpy.types.Scene.bb_imagefilepaths_ap = bpy.props.CollectionProperty(type=bpy.types.PropertyGroup)
bpy.types.Scene.bb_imagefilepaths_ml = bpy.props.CollectionProperty(type=bpy.types.PropertyGroup)


bpy.types.Scene.bb_shift_step = bpy.props.IntProperty \
      (
            name="Shift step",
...
bpy.types.Scene.bb_center_view = bpy.props.BoolProperty \
    (
...
#
bpy.types.Object.bb_image_from_stack_notation = bpy.props.BoolProperty(name = "Image created from Stack Notation tool", default=False)
#*******End comment*******

def update_render_images(self, context):
# Must appear before bpy.types.Scene.render_images declaration
    im_ob_list = [item for item in bpy.data.objects if (item.name =='Image DV' or item.name =='Image AP' or item.name =='Image ML')]
    for im_ob in im_ob_list:
       if(bpy.context.scene.bb_render_images):
            create_plane(im_ob)
       else :
            delete_plane(im_ob)
bpy.types.Scene.bb_render_images = bpy.props.BoolProperty \
    (
...
# find icon names here:  http://wiki.blender.org/index.php/Extensions:2.6/Py/Scripts/3D_interaction/Icon_Tools
    def draw(self, context):
        ...
        row = self.layout.row()
        row.prop(context.scene , "bb_ap_side")
        row.prop(context.scene , "bb_ml_side")
        row.prop(context.scene , "bb_dv_side")

        row = self.layout.row(align=True)
        row.prop(context.scene, "bb_image_path_AP")
        row.operator("bb_importfolder_ap.tif", text='', icon='FILESEL')

        row = self.layout.row(align=True)
        row.prop(context.scene, "bb_image_path_ML")
        row.operator("bb_importfolder_ml.tif", text='', icon='FILESEL')

        row = self.layout.row(align=True)
        row.prop(context.scene, "bb_image_path_DV")
        row.operator("bb_importfolder_dv.tif", text='', icon='FILESEL')
        ...
        colL = split.column()
        colR = split.column()
        colL.operator("bb_superimpose.tif", text='Show Image(s) at Vertex', icon="TEXTURE")  # TEXTURE or IMAGE_DATA
        colR.prop(context.scene , "bb_center_view")

        split = self.layout.row().split(percentage=0.6)
        col1 = split.column()
        col1.operator("object.bb_modal_operator", text='Scroll Through Image Stack', icon="FULLSCREEN_ENTER")
        col2 = split.column().row()
        col2.prop(context.scene , "bb_shift_step")

        row = self.layout.row()
        row.prop(context.scene , "bb_render_images")
...
#*******End comment*******
        ...
        row = self.layout.row()
        row.operator("object.bb_add_transparency", text='Add Transparency')
        row.operator("object.bb_rem_transparency", text='Remove Transpanrency')
        row = self.layout.row()
...
def create_plane(im_ob):
    ap_max = bpy.context.scene.bb_ap_side
    ml_max = bpy.context.scene.bb_ml_side
    dv_max = bpy.context.scene.bb_dv_side
    if("Image DV" in im_ob.name):
        orientation = 'DV'
        pl_dimensions = (ap_max, ml_max, 0)
        pl_location = (ap_max/2, ml_max/2, im_ob.location[2])
        pl_rotation = (3.141592, 0.0, 0.0)
        pl_name = 'Plane DV'
        mat_name = 'Mat DV'
        texture_name = 'Text DV'
    elif("Image AP" in im_ob.name):
        orientation = 'AP'
        pl_dimensions = (dv_max, ml_max, 0)
        pl_location = (im_ob.location[0], ml_max/2, dv_max/2)
        pl_rotation = (0.0, -3.141592/2, 3.141592653)
        pl_name = 'Plane AP'
        mat_name = 'Mat AP'
        texture_name = 'Text AP'
    elif("Image ML" in im_ob.name):
        orientation = 'ML'
        pl_dimensions = (ap_max, dv_max, 0)
        pl_location = (ap_max/2, im_ob.location[1], dv_max/2)
        pl_rotation = (-3.141592/2, 0.0, 0.0)
        pl_name = 'Plane ML'
        mat_name = 'Mat ML'
        texture_name = 'Text ML'
...
def delete_plane(im_ob):
    ...
    if ob_hadPlane:
        if ('Image DV' in im_ob.name):
            mat = bpy.data.materials['Mat DV']
            text = bpy.data.textures['Text DV']
        elif ('Image AP' in im_ob.name):
            mat = bpy.data.materials['Mat AP']
            text = bpy.data.textures['Text AP']
        elif ('Image ML' in im_ob.name):
            mat = bpy.data.materials['Mat ML']
            text = bpy.data.textures['Text ML']
...
def set_texture(im_ob):
    for child in im_ob.children:
        if child.name == 'Plane DV':
            child.data.uv_textures[0].data[0].image = im_ob.data
            child.data.materials['Mat DV'].texture_slots['Text DV'].texture.image = im_ob.data
        elif child.name == 'Plane AP':
            child.data.uv_textures[0].data[0].image = im_ob.data
            child.data.materials['Mat AP'].texture_slots['Text AP'].texture.image = im_ob.data
        elif child.name == 'Plane ML':
            child.data.uv_textures[0].data[0].image = im_ob.data
            child.data.materials['Mat ML'].texture_slots['Text ML'].texture.image = im_ob.data
...
def clear_ims(orientation):
    ...
    if orientation == "AP":
        im_name = "Image AP"
        bpy.context.scene.bb_image_path_AP = "/"
        image_files_full = bpy.context.scene.bb_imagefilepaths_ap

    elif orientation == "ML":
        im_name = "Image ML"
        bpy.context.scene.bb_image_path_ML = "/"
        image_files_full = bpy.context.scene.bb_imagefilepaths_ml

    elif orientation == "DV":
        im_name = "Image DV"
        bpy.context.scene.bb_image_path_DV = "/"
        image_files_full = bpy.context.scene.bb_imagefilepaths_dv

        # Also remove ImageStackLadder if DV
...
class ImageScrollOperator(bpy.types.Operator):
    ...
    bl_idname = "object.bb_modal_operator"
    ...
    def modal(self, context, event):
      ...
      if bpy.context.mode == 'OBJECT' and bpy.context.active_object.type == 'EMPTY':
        ...
        if not bpy.context.active_object.bb_image_from_stack_notation:
        ...
        movement = 1
        if event.shift:
            movement = bpy.context.scene.bb_shift_step
            if (movement <= 0):
                movement = 1
...
# check if image already loaded, only load new image if not
    if (orientation == 'DV'):
        newid = ind+bpy.context.scene.bb_file_min_DV
    elif (orientation == 'AP'):
        newid = ind+bpy.context.scene.bb_file_min_AP
    elif (orientation == 'ML'):
        newid = ind+bpy.context.scene.bb_file_min_ML
...
def getIndex(im_ob):
    ...
    if (imageName == "Image DV"):
        directory = bpy.context.scene.bb_image_path_DV
        exte = bpy.context.scene.bb_image_ext_DV
        image_files = bpy.context.scene.bb_imagefilepaths_dv
        N = len(image_files)
        max=bpy.context.scene.bb_dv_side
        orientation = 'DV'
        point_location = im_ob.location.z
    elif (imageName == "Image AP"):
        directory = bpy.context.scene.bb_image_path_AP
        exte = bpy.context.scene.bb_image_ext_AP
        image_files = bpy.context.scene.bb_imagefilepaths_ap
        N = len(image_files)
        max=bpy.context.scene.bb_ap_side
        orientation = 'AP'
        point_location = im_ob.location.x
    elif (imageName == "Image ML"):
        directory = bpy.context.scene.bb_image_path_ML
        exte = bpy.context.scene.bb_image_ext_ML
        image_files = bpy.context.scene.bb_imagefilepaths_ml
        N = len(image_files)
        max=bpy.context.scene.bb_ml_side
        orientation = 'ML'
...
class SelectStackFolderDV(bpy.types.Operator):  # adjusted
    """Select location of the DV stack images (original image stack)"""
    bl_idname = "bb_importfolder_dv.tif"
    bl_label = "Select folder of the DV stack images"

    directory = bpy.props.StringProperty(subtype="FILE_PATH")
    def execute(self, context):
        clear_ims("DV")
        select_folder_execute(self, "DV", "bb_image_path_DV", "bb_imagefilepaths_dv", "bb_image_ext_DV")
...
#*******End comment*******
        ...
        return {'FINISHED'}
    def invoke(self, context, event):
        select_folder_invoke(self, context, "exte_DV", "bb_image_ext_DV")
        return {"RUNNING_MODAL"}

class SelectStackFolderAP(bpy.types.Operator):  # adjusted
    """Select location of the AP stack images (OPTIONAL)"""
    bl_idname = "bb_importfolder_ap.tif"
    bl_label = "Select folder of the AP stack images"

    directory = bpy.props.StringProperty(subtype="FILE_PATH")
    def execute(self, context):
        clear_ims("AP")
        select_folder_execute(self, "AP", "bb_image_path_AP", "bb_imagefilepaths_ap", "bb_image_ext_AP")
        return {'FINISHED'}
    def invoke(self, context, event):
        select_folder_invoke(self, context, "exte_AP", "bb_image_ext_AP")
        return {"RUNNING_MODAL"}

class SelectStackFolderML(bpy.types.Operator):  # adjusted
    """Select location of the ML stack images (OPTIONAL)"""
    bl_idname = "bb_importfolder_ml.tif"
    bl_label = "Select folder of the ML stack images"

    directory = bpy.props.StringProperty(subtype="FILE_PATH")
    def execute(self, context):
        clear_ims("ML")
        select_folder_execute(self, "ML", "bb_image_path_ML", "bb_imagefilepaths_ml", "bb_image_ext_ML")
        return {'FINISHED'}
    def invoke(self, context, event):
        select_folder_invoke(self, context, "exte_ML", "bb_image_ext_ML")
        return {"RUNNING_MODAL"}
...
def LoadImageFilenames(orientation) :
    """Load the images of the stack indicated by orientation (in 'DV','AP','ML')"""
    image_file_extensions = ["png", "tif", "tiff", "bmp", "jpg", "jpeg", "tga"]  # can add others as needed
    if(orientation == 'DV'):
        imagefilepaths = bpy.context.scene.bb_imagefilepaths_dv
        file_min = bpy.context.scene.bb_file_min_DV
        path = bpy.context.scene.bb_image_path_DV
    elif (orientation == 'AP'):
        imagefilepaths = bpy.context.scene.bb_imagefilepaths_ap
        file_min = bpy.context.scene.bb_file_min_AP
        path = bpy.context.scene.bb_image_path_AP
    elif (orientation == 'ML'):
        imagefilepaths = bpy.context.scene.bb_imagefilepaths_ml
        file_min = bpy.context.scene.bb_file_min_ML
        path = bpy.context.scene.bb_image_path_ML
    ...
    if(orientation == 'DV'):
        bpy.context.scene.bb_file_min_DV = file_min
    elif (orientation == 'AP'):
        bpy.context.scene.bb_file_min_AP = file_min
    elif (orientation == 'ML'):
        bpy.context.scene.bb_file_min_ML = file_min
...
class DisplayImageButton(bpy.types.Operator):  # adjusted
    ...
    bl_idname = "bb_superimpose.tif"
    bl_label = "Superimpose images"

    def execute(self, context):
        if bpy.context.mode == 'EDIT_MESH':
            Nz = len(bpy.context.scene.bb_imagefilepaths_dv)
            if Nz > 0:
                if (bpy.context.active_object.type=='MESH'):
                    DisplayImageFunction('DV')
                else:
                    self.report({'INFO'},"Select a vertex on a mesh object")
            else:
                self.report({'INFO'},"No image files found in the DV directory")
            Nx = len(bpy.context.scene.bb_imagefilepaths_ap)
            if Nx > 0:
                if (bpy.context.active_object.type == 'MESH'):
                    DisplayImageFunction('AP')
                else:
                    self.report({'INFO'}, "Select a vertex on a mesh object")
            else:
                self.report({'INFO'},"No image files found in the AP directory")
            Ny = len(bpy.context.scene.bb_imagefilepaths_ml)
            if Ny > 0:
                if (bpy.context.active_object.type == 'MESH'):
                    DisplayImageFunction('ML')
                else:
                    self.report({'INFO'}, "Select a vertex on a mesh object")
            else:
                self.report({'INFO'},"No image files found in the ML directory")
...
def DisplayImageFunction(orientation):
    ...
    x_max = bpy.context.scene.bb_ap_side
    y_max = bpy.context.scene.bb_ml_side
    z_max = bpy.context.scene.bb_dv_side
    x_min = 0.0
    y_min = 0.0
    z_min = 0.0
    ...
    if (orientation == 'DV'):
        ...
        locs = [delta*n for n in range(N)]  # the dv locations of each image in space
        newName = "Image DV"
        ...
        locs = [delta*n for n in range(N)]  # the ap locations of each image in space
        newName = "Image AP"
        #The rotation that we need to apply on the image.
        rotX = 0
        rotY = -3.141592653/2
        ...
        locs = [delta*n for n in range(N)]  # the ml locations of each image in space
        newName = "Image ML"
        #The rotation that we need to apply on the image.
        rotX = -3.141592653/2
        rotY = 0
    ...
    for v_index in selected_id:
       ...
       im_ob.bb_image_from_stack_notation = True  # denote as being created from this tool, to not be confused with Stack Interactions tool
...
class AddTranspButton(bpy.types.Operator):
    ...
    bl_idname = "object.bb_add_transparency"
...
class RemTranspButton(bpy.types.Operator):
    ...
    bl_idname = "object.bb_rem_transparency"
...
#*******Commented by Tom end around 2875*******
